...
function startNgrokWithPolling() {
  ...
  // ⛔ لو مفيش توكن → تجاهل ngrok تمامًا
  if (!process.env.NGROK_AUTH_TOKEN) {
    console.log("ℹ️ NGROK_AUTH_TOKEN not found — skipping ngrok");
    ngrokDisabled = true;
    return;
  }

  // ⛔ لو ngrok اتعطل قبل كده
  if (ngrokDisabled) {
    console.log("🔕 ngrok disabled — skipping startup");
    return;
  }
  ...
  exec(killCmd, () => {
    exec("ngrok.exe http 3000 --log=stdout", (err, stdout, stderr) => {

      const output = `${stdout || ""} ${stderr || ""} ${err?.message || ""}`;

      // ❌ خطأ مصادقة — قاتل
      if (
        output.includes("ERR_NGROK_4018") ||
        output.includes("authtoken") ||
        output.includes("authentication failed")
      ) {
        console.warn("🔕 Ngrok authentication failed — disabled permanently");
        ngrokDisabled = true;
        return; // ⛔ لا polling ولا retry
      }

      if (err) {
        console.warn("⚠️ Ngrok failed to start — running local only");
        return;
      }
...
app.listen(PORT, () => {
  ...
  // 🟢 افتح التيرمينال المحلي دايمًا
  setTimeout(() => {
    openTerminal(`http://localhost:${PORT}/terminal.html`);
  }, 1200);
});
