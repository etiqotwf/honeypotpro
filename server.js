...
// ---------- Start PowerShell on-demand and pipe output to SSE clients ----------
let psProcessServer = null;
let psStdoutBuffer = '';
let psStderrBuffer = '';

function startPowerShellIfNeeded() {
  if (psProcessServer && !psProcessServer.killed) {
    // already running
    return { started: false, message: 'PowerShell already running' };
  }

  try {
    // spawn a single persistent PowerShell process
    psProcessServer = spawn('powershell.exe', [
      '-NoLogo',
      '-NoExit',
      '-Command',
      '-OutputEncoding', 'utf8'
    ...
    psProcessServer.stdout.on('data', (chunk) => {
      psStdoutBuffer += chunk.toString('utf8');
      const parts = psStdoutBuffer.split(/\r?\n/);
      psStdoutBuffer = parts.pop(); // leftover partial
      for (const line of parts) {
        if (line.trim() === '') continue;
        broadcastLine({ type: 'line', msg: line });
      }
    });

    psProcessServer.stderr.on('data', (chunk) => {
      psStderrBuffer += chunk.toString('utf8');
      const parts = psStderrBuffer.split(/\r?\n/);
      psStderrBuffer = parts.pop();
      for (const line of parts) {
        if (line.trim() === '') continue;
        // use different event type if you want (your client handles 'attack' too)
        broadcastLine({ type: 'attack', msg: line });
      }
    });

    psProcessServer.on('close', (code, signal) => {
      broadcastLine({ type: 'system', msg: `PowerShell exited (code=${code}, signal=${signal})` });
      psProcessServer = null;
    });

    psProcessServer.on('error', (err) => {
      broadcastLine({ type: 'system', msg: `PowerShell spawn error: ${err.message}` });
      psProcessServer = null;
    });

    // optional: send welcome
    broadcastLine({ type: 'system', msg: `PowerShell started at ${new Date().toLocaleString()}` });
    return { started: true, message: 'PowerShell started' };
  } catch (err) {
...
// Endpoint: start PowerShell on-demand
app.post('/start-sse', (req, res) => {
  const result = startPowerShellIfNeeded();
  if (result.started) return res.json({ ok: true, message: result.message });
